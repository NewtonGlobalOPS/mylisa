...
const GENERATOR_VERSION = "canonical-ks2-y3-maths-v1";
...
async function main() {
  ...
  if (!organisationSlug) {
    throw new Error(
      "Usage: npx tsx scripts/generate-ks2-y3-maths-canonical.ts <organisation-slug>"
    );
  }
  ...
  if (objectives.length === 0) {
    throw new Error("No KS2 Year 3 maths objectives found.");
  }
  ...
  for (const objective of objectives) {
    ...
    const questions = generateFromProfile(profile).map((question) => ({
      ...
      generatorMeta: {
        ...
        script: "generate-ks2-y3-maths-canonical",
